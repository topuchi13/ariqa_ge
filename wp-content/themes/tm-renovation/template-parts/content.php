...
<article <?php post_class('post-full-item'); ?>>
	<?php if ( has_post_thumbnail() ) { ?>
		<div class="post-thumb">
			...
			<a href="<?php echo esc_url( get_permalink()); ?>"><?php the_post_thumbnail( array(850, 500, 'bfi_thumb' => true) ); ?></a>
			<?php if ( 'post' == get_post_type() ) : ?>
				<div class="entry-meta">
					<?php TM_Renovation_Templates::posted_on(); ?>
					<span class="author vcard"><i class="fa fa-pencil"></i> <?php echo get_the_author(); ?></span>
					<span class="categories-links"><i class="fa fa-folder-open-o"></i> <?php echo get_the_category_list( esc_html__( ', ', 'tm-renovation' ) ) ?> </span>
	...
	<div class="entry-content">
		<?php TM_Renovation_Templates::print_excerpt(180); ?>
		<?php if (!$infinity_hide_read_more) { ?>
			<p class="read-more"><a href="<?php the_permalink( $post->ID ) ?>"><?php esc_html_e( 'Read More', 'tm-renovation' ); ?><i class="fa fa-angle-double-right"></i></a></p>
		<?php } ?>
	</div>
	<!-- .entry-content -->
