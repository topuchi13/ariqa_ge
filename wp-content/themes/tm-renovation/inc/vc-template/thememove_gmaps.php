				...
				if ( gmMapDiv.length ) {
					...
					gmMapDiv.gmap3( {
						...
						marker: {
							...
							options: {
								<?php if ($marker_icon == '') { ?>
								icon: "<?php echo TM_RENOVATION_URI ?>/images/map-marker.png",
								<?php } else { ?>
							...
							events: {
								click: function( marker, event ) {
									...
									if ( infowindow ) {
										...
										infowindow.setContent( "<?php echo TM_Renovation_Helper::phpStr_to_JSStr( $content ); ?>" );
									}
									else {
										$( this ).gmap3( {
											infowindow: {
												anchor: marker,
												options: { content: "<?php echo TM_Renovation_Helper::phpStr_to_JSStr( $content ); ?>" }
											}
										} );
									}
								}
