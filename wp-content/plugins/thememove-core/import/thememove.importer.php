...
if ( ! class_exists( 'WXR_Parser' ) ) {
	require_once( dirname( __FILE__ ) . DS . 'parsers.php' );
}
...
if ( class_exists( 'WP_Importer' ) ) {
	class TM_WP_Importer extends WP_Importer {
		...
		function __construct( $importFullDemo ) {
			...
			add_action( 'import_end', array( $this, 'update_imported_options' ) );
		...
		function check_writeable() {
			...
			if ( ! is_writable( ABSPATH . 'wp-content' ) ) {
				...
				echo '<script type="text/javascript">' .
				     ...
				     'is_error(\'' .
				     sprintf(
					     wp_kses( __( 'Could not write files into directory: <strong>%swp-content</strong>', 'tm-core' ),
						     array(
							     'strong' => array()
						     )
					     ),
				...
				echo '<script type="text/javascript">>document.getElementById("errorImportMsg").innerHTML += \'<div class="notice notice-success">' .
				     sprintf(
					     wp_kses( __( 'Writable Passed: <strong>%s</strong>wp-content', 'tm-core' ),
						     array(
							     'strong' => array()
						     )
					     ),
				     ...
				     '</div>\';document.getElementById("errorImportMsg").style.display = "inline-block";</script>';
			}

			echo '<br /><form action="" method="post" class="retry-form">' .
			     '<em>' . esc_html__( 'Tips: Try to change Chmode of folder to 755 or 777', 'tm-core' ) . '</em><br/><br/>' .
			     '<input type="submit" id="submitbtn" class="button button-primary" value="' . esc_html__( 'Click to Retry', 'tm-core' ) . '" />' .
			     '<input type="hidden" value="1" name="import_sample_data">' .
			     '<input type="hidden" value="' . esc_attr( $this->demo ) . '" name="demo"/>' .
			     '</form>';
		...
		function download_package( $_tmppath ) {
			...
			if ( ! is_dir( $_tmppath ) ) {
				...
				$demos = $this->importFullDemo ? apply_filters( 'thememove_import_demos', array() ) : apply_filters( 'thememove_import_dummies', array() );
				...
				if ( $this->importFullDemo ) {
					echo '<script type="text/javascript">text_status(\'' .
					     sprintf(
						     wp_kses(
							     __( 'Downloading package ... You can download the package via <a href="%s" target="_blank">this direct link</a>.' ),
							     array(
								     'strong' => array(),
								     'a'      => array(
									     'href'   => array(),
				...
				if ( add_option( 'tm_download_tmp_package', $_tmp ) === false ) {
					update_option( 'tm_download_tmp_package', $_tmp );
				}
				...
				if ( ! is_wp_error( $package ) ) {
						...
						if ( @mkdir( $_tmppath, 0755 ) ) {
							...
							if ( is_wp_error( $unzip ) ) {
								echo '<script type="text/javascript">is_error(\'' .
								     ...
								     sprintf(
									     wp_kses(
										     __( '<strong>ERROR %s:</strong> Could not extract demo media package. Please <a href="%s" target="_blank">contact our support staff</a>.', 'tm-core' ),
										     array(
											     'strong' => array(),
											     'a'      => array(
												     'href'   => array(),
							...
							@unlink( $package );
							delete_option( 'tm_download_tmp_package' );
					...
					echo '<script type="text/javascript">is_error(\'' .
					     ...
					     sprintf(
						     wp_kses(
							     __( '<strong>ERROR %s:</strong> Could not download demo media package. Please use <a href="%s">this direct link</a> or <a href="%s" target="_blank">contact our support staff</a>.', 'tm-core' ),
							     array(
								     'strong' => array(),
								     'a'      => array(
									     'href'   => array(),
		...
		function unpackage( $_cpath, $_tmppath ) {
			...
			if ( is_dir( $_tmppath ) ) {
				...
				echo '<script type="text/javascript">is_error(\'' .
				     ...
				     sprintf(
					     wp_kses(
						     __( '<strong>ERROR %s:</strong> Could not found temporary folder. Please use <a href="%s" target="_blank">contact our support staff</a>.', 'tm-core' ),
						     array(
							     'strong' => array(),
							     'a'      => array(
								     'href'   => array(),
		...
		function get_data( $type, $unserialize = true ) {
			...
			$file = get_template_directory() . DS . 'inc' . DS . 'import' . DS . 'files' . DS . $this->demo . DS . $type . '.txt';
		...
		/**
		 * Get avaiable widgets in current site
		 * @return array
		 */
		...
		function read_widget_logic_file() {
			// Widget Logic
			$widget_logic_file = get_template_directory() . '/inc/import/files/' . $this->demo . '/widget_logic_options.txt';
		...
		function import_sidebars() {
			if ( $this->importFullDemo ) {
				echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Sidebars</b>', 'tm-core' ), array( 'b' => array() ) ) . '\' );</script>';
			}
		...
		function import_widgets() {
			...
			if ( empty( $data ) || ! is_object( $data ) ) {
				echo '<script type="text/javascript">is_error(\'' .
				     ...
				     sprintf(
					     wp_kses(
						     __( '<strong>ERROR %s:</strong> Could not read the widget sample data file. Please <a href="%s" target="_blank">contact our support staff</a>.', 'tm-core' ),
						     array(
							     'strong' => array(),
							     'a'      => array(
								     'href'   => array(),
		...
		function import_customizer_options() {
			...
			if ( $this->importFullDemo ) {
				echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Customizer Options</b>', 'tm-core' ), array( 'b' => array() ) ) . '\' );</script>';
			}
		...
		function import_woocommerce_attributes() {
			...
			$_x_path = get_template_directory() . DS . 'inc' . DS . 'import' . DS . 'files' . DS . $this->demo . DS;
		...
		function import_attribute_swatches() {
			if ( $this->importFullDemo ) {
				echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Attribute Swatches</b>', 'tm-core' ), array( 'b' => array() ) ) . '\' );</script>';
			}
		...
		function import_rev_sliders() {
			...
			if ( $this->importFullDemo ) {
				...
				if ( ! empty( $lines ) ) {
					...
					foreach ( $lines as $line ) {
						...
						if ( substr( trim( $line ), - 1, 1 ) == ';' ) {
							ob_start();
							$wpdb->query( str_replace( array( '%SITE_URI%', 'wp_' ), array(
								$xuri,
								$wpdb->prefix
							), $templine ), false );
							$templine = '';
				...
				$files = glob( get_template_directory() . DS . 'inc' . DS . 'import' . DS . 'files' . DS . $this->demo . DS . 'rev_sliders' . DS . '*.zip' );
				...
				if ( ! empty( $files ) && sizeof( $files ) > 0 ) {
					...
					foreach ( $files as $file ) {
						...
						$revapi->importSliderFromPost( true, true, $file );
		...
		function import_essential_grid() {
			...
			$file = get_template_directory() . '/inc/import/files/' . $this->demo . '/essential_grid.txt';
			...
			try {
				...
				echo '<script type="text/javascript">is_error(\'' .
				     ...
				     sprintf(
					     wp_kses(
						     __( '<strong>ERROR %s:</strong> Could not download demo media package. Please <a href="%s" target="_blank">contact our support staff</a>.', 'tm-core' ),
						     array(
							     'strong' => array(),
							     'a'      => array(
								     'href'   => array(),
		...
		// Update option after imported
		function update_imported_options() {
			$option = TM_THEME_SLUG . '_' . $this->demo . '_imported';
		...
		function dispatch() {
			...
			$_tmppath = $_cpath . TM_THEME_SLUG . '-' . $this->demo . '_tmp';
			...
			// hook before import data
			do_action( 'tm_before_import_data' );
			...
			echo '<script type="text/javascript">progress_status(0);text_status( \'' . esc_html__( 'Preparing for add media...', 'tm-core' ) . '\' );</script>';
			...
			// hook after import data
			do_action( 'tm_after_import_data' );
		}
		...
		function import( $file ) {
			...
			if ( $this->importFullDemo ) {
				echo '<script type="text/javascript">text_status( \'' . esc_html__( 'Get author mapping', 'tm-core' ) . '\' );</script>';
				@ob_flush();
				@flush();
			}
			...
			if ( $this->importFullDemo ) {
				echo '<script type="text/javascript">text_status( \'' . esc_html__( 'Updating incorrect/missing infomation in the database...', 'tm-core' ) . '\');</script>';
				@ob_flush();
				@flush();
			}
		...
		function import_start( $file ) {
			if ( ! is_file( $file ) ) {
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'tm-core' ) . '</strong><br />';
				echo __( 'The file does not exist, please try again.', 'tm-core' ) . '</p>';
				$this->footer();
				die();
			}
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'tm-core' ) . '</strong><br />';
				printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'tm-core' ), $file['file'] );
				echo '</p>';
			...
			if ( $this->version > $this->max_wxr_version ) {
				...
				printf( __( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'tm-core' ), esc_html( $import_data['version'] ) );
				echo '</strong></p></div>';
			}
		...
		function import_options() {
			...
			<form action="<?php echo admin_url( 'admin.php?import=wordpress&amp;step=2' ); ?>" method="post">
				...
				<?php if ( ! empty( $this->authors ) ) : ?>
					<h3><?php _e( 'Assign Authors', 'tm-core' ); ?></h3>
					<p><?php _e( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'tm-core' ); ?></p>
					<?php if ( $this->allow_create_users() ) : ?>
						<p><?php printf( __( 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', 'tm-core' ), esc_html( get_option( 'default_role' ) ) ); ?></p>
					<?php endif; ?>
					<ol id="authors">
				...
				<?php if ( $this->allow_fetch_attachments() ) : ?>
					<h3><?php _e( 'Import Attachments', 'tm-core' ); ?></h3>
					<p>
						<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments"/>
						<label
							for="import-attachments"><?php _e( 'Download and import file attachments', 'tm-core' ); ?></label>
					</p>
				<?php endif; ?>

				<p class="submit"><input type="submit" class="button"
				                         value="<?php esc_attr_e( 'Submit', 'tm-core' ); ?>"/></p>
			</form>
			<?php
		}
		...
		function author_select( $n, $author ) {
			...
			if ( $create_users ) {
				if ( $this->version != '1.0' ) {
					_e( 'or create new user with login name:', 'tm-core' );
					$value = '';
				} else {
					_e( 'as a new user:', 'tm-core' );
			...
			if ( ! $create_users && $this->version == '1.0' ) {
				_e( 'assign posts to an existing user:', 'tm-core' );
			} else {
				_e( 'or assign posts to an existing user:', 'tm-core' );
			}
			wp_dropdown_users( array(
				'name'            => "user_map[$n]",
				'multi'           => true,
				'show_option_all' => __( '- Select -', 'tm-core' )
			) );
			...
			foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
				...
				if ( ! empty( $_POST['user_map'][ $i ] ) ) {
					...
					if ( ! is_wp_error( $user_id ) ) {
						...
						printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'tm-core' ), esc_html( $this->authors[ $old_login ]['author_display_name'] ) );
				...
				if ( 'nav_menu_item' == $post['post_type'] ) {
					...
					if ( $this->importFullDemo ) {
						echo '<script type="text/javascript">text_status( \'' .
						     sprintf(
							     wp_kses(
								     __( 'Adding Custom Post Type <b>%s</b>', 'tm-core' ),
								     array(
									     'b' => array()
								     )
							     ),
		...
		function process_menu_item( $item ) {
			...
			if ( ! $menu_slug ) {
				_e( 'Menu item skipped due to missing menu slug', 'tm-core' );
				echo '<br />';
			...
			if ( ! $menu_id ) {
				printf( __( 'Menu item skipped due to invalid menu slug: %s', 'tm-core' ), esc_html( $menu_slug ) );
				echo '<br />';
			...
			if ( $id && ! is_wp_error( $id ) ) {
				...
				// update item tag
				if ( isset( $_menu_item_tag ) ) {
					update_post_meta( $id, '_menu_item_tag', $_menu_item_tag );
				}

				if ( isset( $_menu_item_tag_type ) ) {
					update_post_meta( $id, '_menu_item_tag_type', $_menu_item_tag_type );
				}

				if ( isset( $_menu_item_icon_classes ) ) {
					update_post_meta( $id, '_menu_item_icon_classes', $_menu_item_icon_classes );
				}

				if ( isset( $_menu_item_tag_color ) ) {
					update_post_meta( $id, '_menu_item_tag_color', $_menu_item_tag_color );
				}

				if ( isset( $_menu_item_tag_bgcolor ) ) {
					update_post_meta( $id, '_menu_item_tag_bgcolor', $_menu_item_tag_bgcolor );
				}

				if ( isset( $_menu_item_tag_bdcolor ) ) {
					update_post_meta( $id, '_menu_item_tag_bdcolor', $_menu_item_tag_bdcolor );
				}

				if ( isset( $_menu_item_tag_color_hover ) ) {
					update_post_meta( $id, '_menu_item_tag_color_hover', $_menu_item_tag_color_hover );
				}

				if ( isset( $_menu_item_tag_bgcolor_hover ) ) {
					update_post_meta( $id, '_menu_item_tag_bgcolor_hover', $_menu_item_tag_bgcolor_hover );
				}

				if ( isset( $_menu_item_tag_bdcolor_hover ) ) {
					update_post_meta( $id, '_menu_item_tag_bdcolor_hover', $_menu_item_tag_bdcolor_hover );
				}
		...
		function process_attachment( $post, $url ) {
			...
			if ( ! empty( $guidraw[1] ) ) {
				$guidraw = SITE_URI . DS . 'wp-content' . $guidraw[1];
			} else {
				$guidraw = SITE_URI . DS . 'wp-content' . $guidraw[0];
			}

			if ( $this->importFullDemo ) {
				echo '<script type="text/javascript">' .
				     'progress_status( ' . ( $this->imageCount / $this->totalImages ) . ' );' .
				     'text_status( \'' . sprintf( __( 'Adding Media %s', 'tm-core' ), $guidraw ) .
				     '\' );</script>';
			...
			if ( ! $this->fetch_attachments ) {
				return new WP_Error( 'attachment_processing_error',
					__( 'Fetching attachments is not enabled', 'tm-core' ) );
			}
			...
			if ( $info = wp_check_filetype( $upload['file'] ) ) {
				...
				return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'tm-core' ) );
			}
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( $code !== 200 ) {
				...
				return new WP_Error(
					'import_file_error',
					sprintf(
						__( 'Remote server returned %1$d %2$s for %3$s', 'tm-core' ),
						$code,
			...
			if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
				...
				return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'tm-core' ) );
			}

			if ( 0 == $filesize ) {
				@unlink( $upload['file'] );

				return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'tm-core' ) );
			}
			...
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				...
				return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s', 'tm-core' ), size_format( $max_size ) ) );
			}
		...
		function greet() {
			...
			echo '<p>' . __( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'tm-core' ) . '</p>';
			echo '<p>' . __( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', 'tm-core' ) . '</p>';
